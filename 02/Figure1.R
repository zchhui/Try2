...
setwd(pat);
de <- read.table("deg1.down.txt",header= T,sep = "\t");
ge <- read.table("deg1.up.txt",header= T,sep = "\t")
hb <- rbind(de,ge)
gene <- rownames(hb)
##候选基因，进行功能富集分析
...
display_number = c(20, 10, 2)#这三个数字分别代表选取的BP、CC、MF的通路条数，这个自己设置就行了
...
rm(list=ls());
gc();
pat <- "D:/Rworkplace2/data2/res/go/down/";
options(stringsAsFactors = F)
setwd(pat);
de <- read.table("deg1.up.txt",header= T,sep = "\t");
gene <- rownames(de)
##候选基因，进行功能富集分析
...
#5、富集到的通路太多了，不方便绘制图片，可以选择部分绘制
display_number = c(20, 20, 9)#这三个数字分别代表选取的BP、CC、MF的通路条数
ego_result_BP <- as.data.frame(ego_BP)[1:display_number[1], ]
ego_result_CC <- as.data.frame(ego_CC)[1:display_number[2], ]
ego_result_MF <- as.data.frame(ego_MF)[1:display_number[3], ]
...
##将以上我们摘取的部分通路重新组合成数据框
go_enrich_df <- data.frame(
  #ID=c(ego_result_BP$res, ego_result_CC$res, ego_result_MF$res), 
  Description=c(ego_result_BP$Description,ego_result_CC$Description,ego_result_MF$Description),
  p.adjust=c(ego_result_BP$p.adjust, ego_result_CC$p.adjust, ego_result_MF$p.adjust),
  type=factor(c(rep("biological process", display_number[1]), 
                rep("cellular component", display_number[2]),
...
ggplot(data=go_enrich_df, aes(x=type_order,y=-log2(p.adjust), fill=type)) + #横纵轴取值
  ...
  xlab("GO term") + 
  ylab("-log2(p.adjust)") + 
  labs(title = "down genes Enriched GO Terms")+
  theme_bw()
